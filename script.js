// script.js — MIDNIGHT WebApp integration (escrow + Track AI)
(function () {
  ...
  const haptic = (t = "impact") => {
    try {
      ...
      if (t === "impact") tg.HapticFeedback.impactOccurred("medium");
      if (t === "succ") tg.HapticFeedback.notificationOccurred("success");
      if (t === "err") tg.HapticFeedback.notificationOccurred("error");
    } catch (e) {}
  };

  // Outside Telegram (e.g. preview, browser test) we still show the app
  // so the AI Track Evaluator is fully usable.
  if (tg) {
    tg.ready();
    tg.expand && tg.expand();
    ...
    $("#user-chip").textContent = "browser";
    const nameEl = $("#name"); if (nameEl) nameEl.textContent = "Browser preview";
    const uidEl = $("#uid"); if (uidEl) uidEl.textContent = "Telegram WebApp not detected";
  }
  ...
  // Mount Track AI module into its page section
  ...
  const pages = ["deal", "trackai", "wallets", "ref", "profile", "lang", "help"];
  function show(p) {
    pages.forEach(x => {
  ...
  if (!tg) return; // Below requires Telegram WebApp
